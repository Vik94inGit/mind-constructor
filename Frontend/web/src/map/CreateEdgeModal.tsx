...
export function CreateEdgeModal({
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiRequestError ? err.message : t.ui.link.failed);
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <Modal title={nodes.length > 2 ? t.ui.link.titleN(nodes.length) : t.ui.link.titleTwo} onClose={onClose}>
      {error && (
      ...
      <form onSubmit={onSubmit}>
        <div className="mb-4 flex flex-col gap-[0.35rem]">
          ...
          >
            <option value="neutral">{t.ui.sentiments.neutral}</option>
            <option value="positive">{t.ui.sentiments.positive}</option>
            <option value="negative">{t.ui.sentiments.negative}</option>
          </select>
        </div>
        <div className="mt-[1.2rem] flex justify-end gap-[0.6rem]">
          ...
          <button
            ...
            disabled={busy}
          >
            {busy ? t.ui.link.linking : pairs.length > 1 ? t.ui.link.createLinks(pairs.length) : t.ui.link.createLink}
          </button>
        </div>
      </form>
    </Modal>
