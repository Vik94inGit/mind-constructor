...
// as a knight's helmet at a glance rather than a blob. The helm itself is
// drawn in the current text color, so the caller still tints it (sentiment
// red/green) by setting `color`; the plume is a fixed light blue instead of
// following that tint, the same way a real helm's plume is its own separate
// color from the steel — it's what keeps the icon reading as a knight's
// helmet rather than a plain colored blob, whichever sentiment color the
// helm itself ends up in.
